    ...
    def _init_db(self):
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Restore today's count so it doesn't reset to 0 on restart
                cursor.execute('''
                    SELECT 
                ...
                if row and row[0] is not None:
                    self.entered_count = row[0]
                    self.exited_count = row[1]
    ...
    def _log_event(self, event_type, track_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO events (timestamp, event_type, track_id) VALUES (datetime('now', 'localtime'), ?, ?)", 
                    (event_type, track_id)
                )
                conn.commit()
        except Exception as e:
