...
class RTSPFrameGrabber:
    """Thread-safe RTSP frame grabber with graceful shutdown (BUG 3 FIX)."""
    ...
    def _update(self):
        """BUG 3 FIX: Wrap read() in try/except to prevent crash on shutdown."""
        while not self.stopped:
            try:
                ...
                # OpenCV C++ exception during shutdown — exit gracefully
                self.stopped = True
                break
    ...
    def stop(self):
        self.stopped = True
        time.sleep(0.1)  # Give the thread a moment to exit read()
        try:
...
def draw_counter_overlay(frame, in_count, out_count, fps, tracker_count):
    ...
    # BUG 6 FIX: Show occupancy (IN - OUT) instead of meaningless IN + OUT
...
# BUG 4 FIX: Create annotators ONCE at module level, not per-frame
_box_annotator = sv.BoxAnnotator(color=sv.ColorPalette.from_hex(["#00BFFF"]), thickness=2)
...
def generate_frames():
    ...
    counter = SingleLineCounter(line_start, line_end, dead_zone=8, cooldown_frames=30, timeout_seconds=5.0)
    ...
    fps_frame_count = 0
    # BUG 2 FIX: Separate counter for debug logging
    debug_frame_count = 0
    # BUG 7 FIX: Track consecutive failed reads for reconnection
    consecutive_failures = 0
    MAX_FAILURES_BEFORE_RECONNECT = 50
    ...
    try:
        while True:
            ...
            if not ret or frame is None:
                ...
                # BUG 7 FIX: Attempt reconnection after too many failures
                if consecutive_failures >= MAX_FAILURES_BEFORE_RECONNECT:
                    print(f"[WARN] {consecutive_failures} consecutive read failures. Attempting reconnect...")
                    grabber.stop()
                    time.sleep(2)
                    grabber = None
                    for url in RTSP_URLS:
                        grabber = RTSPFrameGrabber(url)
                        if grabber.start():
                            print(f"[INFO] Reconnected to {url}")
                            break
                        grabber.stop()
                        grabber = None
                    if grabber is None:
...
if __name__ == "__main__":
    print("Starting Web Server on port 7004...")
    # Run Flask server
